use opencv::{core, imgcodecs, imgproc, ml, prelude::*, types, videoio, Error, Result};
...
const MOTION_STOPPED_AREA: i32 = 10;

// When checking for whether or not motion has stopped, this is
// how many milliseconds to consider
const MOTION_STOPPED_MS: u128 = 1000;

const MIN_POINT_DISTANCE: f32 = 10.0;
const SIMPLIFY_FACTOR: f64 = 5.0;
...
struct Args {
    ...
    /// Enable directionality in glyphs.  Note: Training data must also be generated this way
    #[arg(short, long, default_value_t = true)]
    directionality: bool,
...
fn check_glyph(
    knn: &core::Ptr<dyn KNearest>,
    keypoints: &core::Vector<core::Point>,
    args: &Args,
) -> Result<u32> {
    ...
    knn.find_nearest(&mut sample, 1, &mut result_idx, &mut neighbors, &mut dist)
        .unwrap();
    ...
    if args.train {
        ...
        let val = rng.gen::<u32>();
        imgcodecs::imwrite(
            format!("{}/{}.png", args.new_image_dir, val).as_str(),
            &dest_image,
            &flags,
        )
        .unwrap();
...
fn build_model(args: &Args) -> (core::Ptr<dyn KNearest>, HashMap<u32, String>) {
    ...
    let training_paths: Vec<_> = fs::read_dir(args.image_dir.clone())
        .unwrap()
        .map(|dir| dir.unwrap())
        .collect();
...
fn draw_glyph(dest_image: &mut core::Mat, points: &core::Vector<core::Point>) {
    ...
    let color_delta = 200.0 / (simplified.len() - 1) as f64;
    ...
    for keypt in simplified.iter().skip(1) {
        ...
        imgproc::line(
            ...
            prev_pt,
            keypt,
            core::VecN([color_val, color_val, color_val, 1.0]),
            2,
            0,
            0,
        )
...
fn add_spaced_points(pts: &mut core::Vector<core::Point>, pt1: core::Point2f, pt2: core::Point2f) {
    let dist = ((pt1.x - pt2.x).powi(2) + (pt1.y - pt2.y).powi(2)).sqrt();
    // No need to add intermediate points
    if dist < MIN_POINT_DISTANCE {
        return;
    }
    // Get the midpoint
    let mid_pt = core::Point2f::new((pt1.x + pt2.x) / 2.0, (pt1.y + pt2.y) / 2.0);
    // Recursively add more midpoints until we hit our SIMPLIFY epsilon
    // From the first point to the midpoint
    add_spaced_points(pts, pt1, mid_pt);
    // Now add the midpoint
    let new_pt = core::Point::new(mid_pt.x as i32, mid_pt.y as i32);
    pts.push(new_pt);
    // Now from the second point to the midpoint
    add_spaced_points(pts, mid_pt, pt2);
    // Now add the second point
    let new_pt = core::Point::new(pt2.x as i32, pt2.y as i32);
    pts.push(new_pt);
}
    ...
    loop {
        ...
        loop {
            ...
            if let Some(center_point) = get_max_contour_center(&found_contours) {
                if all_keypoints.len() > 0 {
                    // Add evenly spaced intermediate points.  We want to do this so that the
                    // grayscale shading is as consistent as we can make it for directionality
                    let last_pt = all_keypoints.get(all_keypoints.len() - 1).unwrap().clone();
                    let pt1 = core::Point2f::new(last_pt.x as f32, last_pt.y as f32);
                    let pt2 = core::Point2f::new(center_point.x as f32, center_point.y as f32);
                    add_spaced_points(&mut all_keypoints, pt1, pt2);
                } else {
                    all_keypoints.push(center_point);
                }
                // Add our new keypoint
